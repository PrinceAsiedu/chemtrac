...
from .models import Formula, Chemical
from .forms import FormulaForm, FormulaChemicalForm

def formula_list(request):
    formulas = Formula.objects.all()
    return render(request, 'inventory/formula_list.html', {'formulas': formulas})
...
def formula_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('inventory:formula_detail', pk=formula.pk)
    else:
        form = FormulaChemicalForm()
    return render(request, 'inventory/formula_detail.html', {'formula': formula, 'form': form})
...
def add_formula(request):
    if request.method == 'POST':
        ...
        form = FormulaForm()
    return render(request, 'inventory/formula_form.html', {'form': form})
...
def remove_chemical(request, formula_pk, chemical_pk):
    ...
    return redirect('inventory:formula_detail', pk=formula_pk)
